...
#define _BB_PAGE_ALLOC_ORDER_LO(page_count)\
	(size_t)floorf(log2f((float)(page_count)))

#define _BB_PAGE_ALLOC_ORDER_HI(page_count)\
	(size_t)ceilf(log2f((float)(page_count)))
...
void* bb_page_allocator_allocate(bb_page_allocator_t* allocator, size_t page_count) {
	...
	const size_t pages_in_zone = (1ull << order);
	...
	for (int z = (int)allocator->_max_order; z >= 0 && pages_to_relocate; z--) {
		const size_t pages = (1ull << z);
		if (pages <= pages_to_relocate) {
			...
			pages_to_relocate -= pages;
			prev_page_ptr = realloc_page_ptr;
			realloc_page_ptr += (pages * kAllocAlign_4k);
		}
	}
	return ptr;
}
...
static void _dump_bb_allocator(bb_page_allocator_t* allocator) {
	...
	for (size_t z = 0; z <= allocator->_max_order; ++z) {
		...
		if (allocator->_zones[z]) {
			...
			printf("] %llu pages at 0x%llx\n", pages_in_zone, allocator->_zones[z]);
		}
		else {
			printf("]\n");
		}
	}
	printf("found %llu pages for a total of %llu bytes\n", pages_available, pages_available * kAllocAlign_4k);
}
