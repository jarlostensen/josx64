...
static _JOS_ALWAYS_INLINE bool str_is_empty(const char* str) {
    return !str || str[0]==0;
}

static _JOS_ALWAYS_INLINE bool slice_is_empty(const char_array_slice_t* slice) {
    return !slice || slice->_length == 0 || slice->_ptr == 0;
}

static _JOS_ALWAYS_INLINE char_array_slice_t* char_array_slice_create(char_array_slice_t* slice, const char* charray, size_t offset, size_t length) {
