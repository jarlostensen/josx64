...
PATH=%CD%\qemu
cd .\external\qemu
qemu-system-x86_64.exe -m 512 -smp 2 -L OVMF_dir/ -bios ..\OVMF-X64-r15214\OVMF.fd -drive format=raw,file=..\..\build\boot.dd,if=ide -vga std -display sdl
rem -nodefaults -rtc base=localtime,clock=vm -vga std -global driver=cfi.pflash01,property=secure,value=on -global driver=cfi.pflash01,property=secure,value=on -global ICH9-LPC.disable_s3=1 -serial file:..\..\qemu_serial.txt
