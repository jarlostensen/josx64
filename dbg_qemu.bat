...
.\external\qemu.exe -s -S -m 512 -smp 2 -L OVMF_dir/ -bios .\external\OVMF-X64-r15214\OVMF.fd -drive format=raw,file=build\boot.dd,if=ide -nodefaults -vga std -global driver=cfi.pflash01,property=secure,value=on -global driver=cfi.pflash01,property=secure,value=on -global ICH9-LPC.disable_s3=1 -serial file:qemu_serial.txt
